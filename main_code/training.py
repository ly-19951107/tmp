...
def train_model(task_id: str, info: list, logger):
    """模型训练"""
    one = train_state_one(task_id, info, logger)
    if not one:
        return 0
    three = train_state_three(task_id, info, logger)
    if not three:
        return 0
...
def train_state_one(task_id: str, info: list, logger):
    logger.info("最大风能捕获区的模型训练...")
    data = pd.read_csv(os.path.join(project.preprocessed_dir, f'train_data_1_{task_id}.csv'),
                       header=0, encoding='utf-8')
    if data.empty:
        logger.warning("最大风能捕获区无数据，无法进行训练")
        return 0
    logger.info("数据归一化")
    x, y = select_feature(data, "one", info)
    logger.info("模型训练及保存")
    train_and_save(x, y, "one", info)
    return 1


def train_state_three(task_id: str, info: list, logger):
    logger.info('恒定功率区的模型训练')
    data = pd.read_csv(os.path.join(project.preprocessed_dir, f"train_data_3_{task_id}.csv"),
                       header=0, encoding='utf-8')
    if data.empty:
        logger.warning("恒定功率区无数据，无法进行训练")
        return 0
    logger.info("数据归一化")
    x, y = select_feature(data, "three", info)
    logger.info("模型训练及保存")
    train_and_save(x, y, "three", info)
    return 1


def select_feature(data: pd.DataFrame, state: str, info: list):
    ver = info[2]
    data = data.iloc[:, 8:]
    y = data['齿轮箱油温_avg']
    ...
    mms = MinMaxScaler()
    mms.fit(x)
    joblib.dump(mms, os.path.join(project.feature_dir, f"{state}_mms_v{ver}.m"))
    x = mms.transform(x)
    return x, y


def train_and_save(x: pd.DataFrame, y: pd.DataFrame, state: str, info: list):
    x_train, x_test, y_train, y_test = train_test_split(x, y, test_size=.4)
    rf = RandomForestRegressor(n_estimators=10, oob_score=True, random_state=10, n_jobs=-1)
    rf.fit(x_train, y_train.values.ravel())
    ver = info[2]
    joblib.dump(rf, os.path.join(project.model_dir, f"{state}_rf_v{ver}.m"))
    y_test_predict = rf.predict(x_test)
    loss = pd.DataFrame(y_test.to_list() - y_test_predict, columns=['loss_data'])
    loss.to_csv(os.path.join(project.threshold_dir, f"{state}_v{ver}.csv"),
